...
public:
    ...
    Vector3(const double* v) : x(v[0]), y(v[1]), z(v[2]) {}
...
// Transform structure with matrix conversion
struct FTransform {
    fquat rot;
    Vector3 translation;
    char pad[4];
...
// Main engine class with core functionality
struct unrealengine {
    struct FQ {
        double x;
        double y;
    ...
    __forceinline auto screencheck(Vector2 location) {
        return (location.x > 0 && location.x < globals->width && location.y > 0 && location.y < globals->height);
    }
    ...
    __forceinline auto w2s(Vector3 worldlocation) -> Vector2 {
        ...
        return Vector2(
            (globals->width / 2.0f) + vTransformed.x * (((globals->width / 2.0f) / tanf(camera.fov * (float)M_PI / 360.f))) / vTransformed.z,
            (globals->height / 2.0f) - vTransformed.y * (((globals->width / 2.0f) / tanf(camera.fov * (float)M_PI / 360.f))) / vTransformed.z
        );
    }

    // World to screen 3D coordinate conversion
