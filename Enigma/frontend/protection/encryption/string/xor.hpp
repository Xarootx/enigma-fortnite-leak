...
// Compiler-specific inline directives
...
// Helper macros for type operations
...
// Generate encryption key based on compile time
XORSTR_CONST_NOINLINE
...
// Main XOR string encryption class
...
private:
    ...
    static XORSTR_CONST_INLINE _CHAR_TYPE_ CRYPT_CHAR(
...
// Helper functions for different character types
template<SIZE_T _STR_LEN_>
XORSTR_CONST_INLINE
_XORSTR_<CHAR, _STR_LEN_> XorStr(IN CHAR CONST(&String)[_STR_LEN_]) {
    return _XORSTR_<CHAR, _STR_LEN_>(String);
}

template<SIZE_T _STR_LEN_>
XORSTR_CONST_INLINE
_XORSTR_<WCHAR, _STR_LEN_> XorStr(IN WCHAR CONST(&String)[_STR_LEN_]) {
    return _XORSTR_<WCHAR, _STR_LEN_>(String);
}

template<SIZE_T _STR_LEN_>
XORSTR_CONST_INLINE
_XORSTR_<char32_t, _STR_LEN_> XorStr(IN char32_t CONST(&String)[_STR_LEN_]) {
